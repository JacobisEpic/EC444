...
static void timer_evt_task(void *arg) {
  while (1) {
    ...
    if (xQueueReceive(timer_queue, &ele, pdMS_TO_TICKS(2000)) && counting) {
      timer--;
      printf("timer is: %d\n", timer);
      flag = true;           // Set a flag to be used elsewhere 
      if (timer == 0) {
        counting = false; // stop counting once timer reaches the end
        printf("timer Finshed\n");
        // Turn on the buzzer (make it buzz)
        gpio_set_level(BUZZER_GPIO, 1);
        vTaskDelay(5000 / portTICK_PERIOD_MS); // Buzz for 1 second
...
static void test_alpha_display() {
    ...
    while (1) {
        ...
        // displaybuffer[1] = alphafonttable[onesPlaceMin];
        // displaybuffer[3] = alphafonttable[onesPlaceMin];
        // displaybuffer[2] = alphafonttable[onesPlaceMin];
        // displaybuffer[0] = alphafonttable[onesPlaceMin];
