...
// Receive task -- looks for Start byte then stores received values
void recv_task(){
    ...
    if (len_in > 0) {
      ...
      if (nn < len_in - len_out + 1) {
        ...
        if (checkCheckSum(copied,len_out)) {
          char receivedColor = copied[1];
          if (receivedColor == 'R' || receivedColor == 'G' || receivedColor == 'Y') {
            myColor = receivedColor;
            printf("Received Color: %c\n", receivedColor);
          }
        }
      }
    }
