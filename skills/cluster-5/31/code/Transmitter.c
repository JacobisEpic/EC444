...
// Tasks 
// Button task -- rotate through myIDs and change LED color
void button_task(){
  uint32_t io_num;
  while(1) {
    if(xQueueReceive(gpio_evt_queue, &io_num, portMAX_DELAY)) {
      ...
      // Change LED color
      switch(myColor) {
        case 'R': myColor = 'G'; break;
        case 'G': myColor = 'Y'; break;
        case 'Y': myColor = 'R'; break;
        default:  myColor = 'R'; // Default case
      }

      xSemaphoreGive(mux);
      printf("Button pressed. ID: %d, Color: %c\n", myID, myColor);
    }
    vTaskDelay(100 / portTICK_PERIOD_MS);
  }
}

// Send task -- sends payload | Start | myColor | myID | Checksum
void send_task(){
  while(1) {
