...
uint16_t char_to_display_val(char c) {
    if (c >= ' ' && c <= '~') {
        return nums[c - ' '];
    }
    return 0;
...
static void test_alpha_display() {
    ...
    while (1) {
        ...
        fgets(inputBuffer, sizeof(inputBuffer) - 3, stdin);
        strtok(inputBuffer, "\n");
        printf("%s\n", inputBuffer);
        ...
        if (length <= 4) {
            for (int i = 0; i < 4; i++) {
                if (i < length) {
                    displaybuffer[i] = char_to_display_val(inputBuffer[i]);
                } else {
                    displaybuffer[i] = 0;
                }
            }
            ...
            while(1) {
                for (int offset = 0; offset < length - 3; offset++) {
                    for (int i = 0; i < 4; i++) {
                        if (i + offset < length) {
                            displaybuffer[i] = char_to_display_val(inputBuffer[i + offset]);
                        } else {
                            displaybuffer[i] = 0;
                        }
                    }
                    ...
                    if (uart_read_bytes(UART_NUM_0, (uint8_t *)inputBuffer, 1, 20 / portTICK_PERIOD_MS) > 0) {
                        break;
                    }
                }
                if (uart_read_bytes(UART_NUM_0, (uint8_t *)inputBuffer, 1, 20 / portTICK_PERIOD_MS) > 0) {
