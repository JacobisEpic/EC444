...
int timer = 0;
int mode = 0; // mode 0 is clock, mode 1 is stopwatch, mode 2 is timer mode 3 is leaderboard 
...
int displayNum = 0;
char winner[2];
uint16_t winnerTable[2] = {0b0000000011110111, // A
...
static void button_function(){
    while(1) {
        ...
        if ((gpio_get_level(btn) == 1) && (mode % 4 == 1)){     // mode 1 
            presses++;      
        ...
        if (gpio_get_level(btn) == 1 && (mode % 4 == 2)){       // mode 2
            timerCounting = true;
            flag  = !flag;
            timer = timer + 30;        
        }
...
static void test_alpha_display() {
    ...
    while (1) {
        ...
        if (mode % 4 == 3) {    // leaderboard mode
            if (winner[0] == 'A')
            {
                displaybuffer[0] = winnerTable[0];
            }
            else
            {
                displaybuffer[0] = winnerTable[1];
            }
            displaybuffer[1] = 0b0000000000000000;         // empty display
            displaybuffer[2] = 0b0000000000000000;         // empty display
            displaybuffer[3] = 0b0000000000000000;         // empty display
        }
        // send count characters / winner to display over I2C
        if (mode % 4 == 1 || mode % 4 == 2 || mode % 4 == 3) 
        {
...
static void read_temp(){
  ...
  stienhart = temp / THERMISTORNOMINAL;
  stienhart = log(stienhart);
  stienhart /= BCOEFFICIENT;
  stienhart += 1.0 / (TEMPERATURENOMINAL + 273.15);
  stienhart = 1.0 / stienhart; 
  stienhart -= 273.15;
  ...
  if (countTime % 5 == 0){
    float range = 5.0;
    float min = 27.0;
    steinhart = min + randNum * range;
    steinhart = (int)(steinhart * 1000) / 1000.0f; 
  }
  else{
    float range = 2.0;
    float min = 25.0;
...
// udp client function to communicate with the server
static void udp_client_task(void *pvParameters)
{
    ...
    while (1) {
        ...
        while (1) {
            //Initializes payload here
            //the format is: Data: {temperature} {numSteps}
            //Send numSteps and steinhart variables as a message to the server
            //If it is userA sending data it will be UserA: Temp numSteps, UserB will be vice versa
            snprintf(payload, sizeof(payload), "UserA: %.4f %d", steinhart, numSteps);
            printf("%.4f\n", steinhart);
            printf("%d\n", numSteps);
            ...
            // Data received
            // Time is sent from server and it converts in to the ESP32 board
            else {
                ...
                if (strncmp(rx_buffer, "OK: ", 4) == 0) {
                    ...
                    *(time + 3) = *(rx_buffer + 4);
                    *(time + 4) = '\0';
                    winner[0] = *(rx_buffer + 9);
                    winner[1] = '\0';
                    printf("%s\n",time);
                    printf("%s\n",winner);
                    str_to_bin(time, 4);
                    if (mode % 4 == 0)      // clock mode
                    {
