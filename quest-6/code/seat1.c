...
static void mqtt5_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_DATA:
        ...
        if (strncmp("/topic/seat1/receive", event->topic, event->topic_len) == 0) {
...
static void mqtt5_app_start(void)
{
    ...
    esp_mqtt_client_subscribe(client, "/topic/seat1/receive", 0);
    ...
    while (1) {
        ...
        if (steinhart > 32 && reservation_status == '0'){
            ...
            blink_led('R');
            char payload[30];
            snprintf(payload, sizeof(payload), "1,%f,%c,%c", steinhart, 'R',reservation_status);
            esp_mqtt_client_publish(client, "/topic/seat1/send", payload, 0, 1, 1);
        }
        else if (steinhart < 32 && reservation_status == '0'){
            // Not reserved and no heat detected
            blink_led('G');
            char payload[30];
            snprintf(payload, sizeof(payload), "1,%f,%c,%c", steinhart, 'G',reservation_status);
            esp_mqtt_client_publish(client, "/topic/seat1/send", payload, 0, 1, 1);
        }
