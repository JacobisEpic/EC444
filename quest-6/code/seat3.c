...
char reservation_status = '0'; // 0 for not reserved and 1 for reserved
...
static void mqtt5_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_DATA:
        // Check if the received message is on the "/topic/qos2" topic
        if (strncmp("/topic/seat3/receive", event->topic, event->topic_len) == 0) {
            reservation_status = event->data[0];  
        }
        print_user_property(event->property->user_property);
        break;
...
static void mqtt5_app_start(void)
{
    ...
    esp_mqtt_client_config_t mqtt5_cfg = {
        .broker.address.uri = "seatoccupancy.duckdns.org:1883",
    ...
    esp_mqtt_client_subscribe(client, "/topic/seat3/receive", 0);
    ...
    while (1) {
        ...
        if (steinhart > 32 && reservation_status == '0'){
            // Not reserved and heat detected
            blink_led('R');
            char payload[30];
            snprintf(payload, sizeof(payload), "3,%f,%c,%c", steinhart, 'R',reservation_status);
            esp_mqtt_client_publish(client, "/topic/seat3/send", payload, 0, 1, 1);
        }
        else if (steinhart < 32 && reservation_status == '0'){
            // Not reserved and no heat detected
            blink_led('G');
            char payload[30];
            snprintf(payload, sizeof(payload), "3,%f,%c,%c", steinhart, 'G',reservation_status);
            esp_mqtt_client_publish(client, "/topic/seat3/send", payload, 0, 1, 1);
        }
            ...
            char payload[30];
            snprintf(payload, sizeof(payload),"3,%f,%c,%c", steinhart, 'R', reservation_status);
            // ESP_LOGI(TAG, "1,%f,%c,%s", steinhart, 'R', received_message);
            esp_mqtt_client_publish(client, "/topic/seat3/send", payload, 0, 1, 1);
        }
